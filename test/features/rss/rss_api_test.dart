...
void main() {
  test(
    'loads feeds and aggregated posts from public and admin endpoints',
    () async {
      final adapter = RssAdapter(
        '{"code":200,"data":{"items":[{"id":1,"name":"Feed","rss_url":"https://feed.test","friend_link_id":-1,"times":0,"status":"valid","is_died":false,"updated_at":1}],"total":1,"page":1,"page_size":20}}',
      );
      final api = RssApi(
        ApiClient(
      ...
      expect(
        ...
        'https://api.test/api/action/rss?page=1&page_size=20',
      );

      final postsAdapter = RssAdapter(
        '{"code":200,"data":{"items":[{"id":2,"rss_id":1,"title":"Post","link":"https://post.test","description":"D","author":"A","time":3}],"total":1,"page":1,"page_size":20}}',
      );
      final postsApi = RssApi(
        ApiClient(
          baseUrl: 'https://api.test',
          store: MemorySecureStore(),
          dio: Dio()..httpClientAdapter = postsAdapter,
        ),
      );
      final posts = await postsApi.listPosts();
      expect(posts.items.single.title, 'Post');
      expect(postsAdapter.request?.method, 'GET');
      expect(
        postsAdapter.request?.uri.toString(),
  ...
  test('uses refresh, fetch, and delete endpoints', () async {
    final adapter = RssAdapter(
      '{"code":200,"data":{"checked_items":2,"inserted_items":1}}',
    );
    final api = RssApi(
      ApiClient(
        baseUrl: 'https://api.test',
